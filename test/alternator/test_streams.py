...
def compare_events(expected_events, output, mode):
    ...
    for event in output:
        ...
        if type(expected_type) is list: # relaxed op checking to bypass #6918
...
def test_streams_putitem_new_item_overrides_old(test_table_ss_new_and_old_images, dynamodb, dynamodbstreams):
    def do_updates(table, p, c):
        events = []
        table.put_item(Item={'p': p, 'c': c, 'a': 1})
        events.append(['INSERT', {'p': p, 'c': c}, None, {'p': p, 'c': c, 'a': 1}])
...
# Test a single PutItem of a new item in a table with no clustering key.
def test_streams_putitem_no_ck_keys_only(test_table_s_no_ck_keys_only, dynamodbstreams):
    def do_updates(table, p, _):
        events = []
        table.put_item(Item={'p': p, 'x': 2})
        events.append(['INSERT', {'p': p}, None, None])
        return events
    do_test(test_table_s_no_ck_keys_only, dynamodbstreams, do_updates, 'KEYS_ONLY')

def test_streams_putitem_no_ck_new_item_overrides_old(test_table_s_no_ck_new_and_old_images, dynamodb, dynamodbstreams):
    def do_updates(table, p, _):
        events = []
...
# This is the same test as test_streams_putitem_no_ck_new_items_override_old.
# Due to issue #6918, the last two mutations are INSERTs, but should be MODIFY.
# Upon fixing the issue, this test should be removed.
def test_streams_putitem_no_ck_new_items_override_old_2(test_table_s_no_ck_new_and_old_images, dynamodbstreams):
    def do_updates(table, p, _):
        events = []
        table.put_item(Item={'p': p, 'a': 1})
...
# Test a single UpdateItem. Should result in a single INSERT event. Replicates
# #6918.
@pytest.mark.xfail(reason="Currently fails - see issue #6918")
def test_streams_updateitem_keys_only(dynamodb, test_table_ss_keys_only, dynamodbstreams):
    def do_updates(table, p, c):
        events = []
        table.update_item(Key={'p': p, 'c': c},
...
@pytest.mark.xfail(reason="Currently fails - because of multiple issues listed above")
def test_streams_1_new_and_old_images(dynamodb, test_table_ss_new_and_old_images, dynamodbstreams):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
